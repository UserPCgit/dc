...
const googleApiJson = process.env.GOOGLE_SERVICE_ACCOUNT_JSON;

if (!googleApiJson) {
  console.error('Google API JSON environment variable is not set.');
  process.exit(1);
}

// Сохраняем содержимое JSON в временный файл или используем напрямую
const credentials = JSON.parse(googleApiJson);
...
// Вход в Discord
client.login(process.env.DISCORD_BOT_TOKEN); // Замените на ваш токен бота
