...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.customId === 'get_user_info') {
    ...
    try {
      ...
      const range = 'w!A:D'; // Убедитесь, что это имя вашего листа
      ...
      if (rows && rows.length) {
        // Ищем ID в четвертом столбце (row[3])
        const userInfo = rows.find(row => row[3] === userId);
        if (userInfo) {
          await interaction.reply({ content: `Информация о пользователе: ${userInfo.join(', ')}`, ephemeral: true });
        } else {
